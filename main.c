...
unsigned char send_data_structure[] = {
	0x41, // TCON HI byte
	0xFF, // TCON LO byte W, A, B ポートは全て接続した状態にする。ハード的にはBポートはグランドに落とす
	0x10, // VOLATILE Wiper 1 HI byte
	0x00, // VOLATILE Wiper 1 LO byte registance 100kΩ
	0x10, // VOLATILE Wiper 1 HI byte
	0x40, // VOLATILE Wiper 1 LO byte registance 50kΩ
};
...
void main (void) {
	...
	//メインルーチン記載エリア---------------------------------
		...
		if( port_chg == 1 )
		{
			...
			if(port_val == 0)
			{
				send_data_to_potensiometer(1); // SPIでデータ送信
			}
			else // port_val == 1
			{
				send_data_to_potensiometer(2); // SPIでデータ送信
			}
		}
	}
}
...
/*
		// テストポートぱたぱた制御
		// RA1 を約3秒毎にHIGH LOWを繰り返す
		// ⇒OK 
		port_test_cnt += 1;
		if( port_test_cnt > 50  ) // 3000だと95秒
		{
			port_test_cnt = 0;

			RB5 = 0;							// CSセット
			send_cnt = 1;						// 送信回数セット
			SSPBUF = send_data_structure[6];	// データ送信
		}
*/
...
//*/			
		...
		{
			SSPBUF = send_data_structure[send_data*2 + 1];	// データ送信(2回目)
		}
	}
}
...
{
	RB5 = 0;									// CSセット
	send_data = data;							// 送信データ情報取得
	send_cnt = 2;								// 送信回数セット
	SSPBUF = send_data_structure[send_data*2];	// データ送信
/*	
			// KUBOTA TEST ① -> OK
			if(RA1 == 0)
			{
...
void spi_mode_init(void)
{
	 ...
	 PORTA  = 0b00000010 ;     // 出力ピンの初期化(全てLOWにする) ->変更：RA1は、テストピン使用する。(初期値：HIとした)
	 PORTB  = 0b00100000 ;     // 出力ピンの初期化(全てLOWにする) ->変更：RB5は、SPIモードのCSとして使用する。(初期値：HIとした)
...
//   ⇒   どうつながるかを幾井君に聞く。それによってTCONセッティング方法が異なる
